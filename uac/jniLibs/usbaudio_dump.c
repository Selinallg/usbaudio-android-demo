...
static jclass com_luoxi_uac_AudioPlayback = NULL;
static jmethodID com_luoxi_uac_AudioPlayback_write;
...
static void cb_xfr(struct libusb_transfer *xfr)
{
    ...
    // Call write()
    (*env)->CallStaticVoidMethod(env, com_luoxi_uac_AudioPlayback,
            com_luoxi_uac_AudioPlayback_write, audioByteArray);
...
Java_com_luoxi_uac_UsbAudio_measure(JNIEnv* env UNUSED, jobject foo UNUSED) {
    return measure();
}
...
JNIEXPORT void JNI_OnUnload(JavaVM* vm, void* reserved UNUSED)
{
    ...
    (*env)->DeleteGlobalRef(env, com_luoxi_uac_AudioPlayback);

    LOGD("libusbaudio: unloaded");
}

JNIEXPORT jboolean JNICALL
Java_com_luoxi_uac_UsbAudio_setup(JNIEnv* env UNUSED, jobject foo UNUSED)
{
	int rc;
	...
	devh = libusb_open_device_with_vid_pid(NULL, 0x1E2F, 0x9785);
	if (!devh) {
    ...
    jclass clazz = (*env)->FindClass(env, "com/luoxi/uac/AudioPlayback");
    if (!clazz) {
        LOGD("Could not find au.id.uac.AudioPlayback");
        libusb_close(devh);
        libusb_exit(NULL);
        return false;
    }
    com_luoxi_uac_AudioPlayback = (*env)->NewGlobalRef(env, clazz);

    com_luoxi_uac_AudioPlayback_write = (*env)->GetStaticMethodID(env,
            com_luoxi_uac_AudioPlayback, "write", "([B)V");
    if (!com_luoxi_uac_AudioPlayback_write) {
        LOGD("Could not find au.id.uac.AudioPlayback");
        (*env)->DeleteGlobalRef(env, com_luoxi_uac_AudioPlayback);
        libusb_close(devh);
        libusb_exit(NULL);
...
Java_com_luoxi_uac_UsbAudio_stop(JNIEnv* env UNUSED, jobject foo UNUSED) {
    do_exit = 1;
    measure();
}

JNIEXPORT bool JNICALL
Java_com_luoxi_uac_UsbAudio_close(JNIEnv* env UNUSED, jobject foo UNUSED) {
    if (do_exit == 0) {
        return false;
    }
...
JNIEXPORT void JNICALL
Java_com_luoxi_uac_UsbAudio_loop(JNIEnv* env UNUSED, jobject foo UNUSED) {
	while (!do_exit) {
